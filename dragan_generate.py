...
def build_generator():
    ...
    model.add(Conv2DTranspose(256, 4, strides=2,
                              padding="same",
                              use_bias=False, kernel_initializer=conv_init))  # 8x8x256
    model.add(Activation("relu"))
    model.add(Conv2DTranspose(128, 4, strides=2,
                              padding="same",
                              use_bias=False, kernel_initializer=conv_init))  # 16x16x128
    model.add(Activation("relu"))
    model.add(Conv2DTranspose(64, 4,  strides=2,
                              padding="same",
                              use_bias=False, kernel_initializer=conv_init))  # 32x32x64
    model.add(Activation("relu"))
    model.add(Conv2DTranspose(3, 4,  strides=2,
                              padding="same",
                              use_bias=False, kernel_initializer=conv_init))  # 64x64x3
...
def main():
    ...
    gen_imgs = gen_imgs.reshape(
        gen_imgs.shape[0], gen_imgs.shape[2], gen_imgs.shape[3], gen_imgs.shape[1])
    ...
    for i in range(r):
        for j in range(c):
            ...
            axs[i, j].axis('off')
            cnt += 1
    fig.savefig("result_img/epoch2900.png")
    plt.close()
